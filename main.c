  ...
  { 
    ...
    case 13:
		...
		for(int i = 0; i < b.sizeOfBoard; i++)
		{
			...
			sum += b.cells[i].impr.numOfHouses * 40 + b.cells[i].impr.numOfHotels * 115; //Pay $40 per house and $115 per hotel you own
			}
		}
		subtractMoney(p, sum);
      	break;
  ...
  else
  {
    ...
    case 3: //Advance token to nearest Utility. 
			...
			//If owned, throw dice and pay owner a total 10 times the amount thrown.
      		//Electric Company, Reading Railroad, B. & O. Railroad, Pennsylvania Railroad, Short Line, Water Works
		bool out_of_bound =true;
		for(int i = p.position; i < b.sizeOfBoard; i++){
			if(b.cell[i].title == "Electric Company" || b.cell[i].title == "Water Works" || b.cell[i].title == "Reading Railroads" || b.cell[i].title == "B. & O. Railroad" || b.cell[i].title == "Pennsylvania Railroad" || b.cell[i].title == "Short Line")
			{
				movePlayer(p,i);
				out_of_bound=false;
				if(b.cells[i].owner == null){
					int choice;
					printf("Will you buy or not? Press 1 to buy, or 2 to reject");
					if(choice==1){
						subtractMoney(p, b.cells[i].price);
						b.cells[i].owner = p;
					}
					else
						continue;
					...
					// we have access to global Dice d variable
					Randomize(struct Dice d, struct Board b, struct Player p);
					// we can access num1, num2 of global Dice d
					int total = (d.num1 + d.num2) * 10;
					subtractMoney(p, total);
					giveMoney(b.cells[i].owner, total);
				}
			}
		}
		if(out_of_bound==true){
			for(int i = 0; i < p.sizeOfBoard; i++){
				f(b.cell[i].title == "Electric Company" || b.cell[i].title == "Water Works" || b.cell[i].title == "Reading Railroads" || b.cell[i].title == "B. & O. Railroad" || b.cell[i].title == "Pennsylvania Railroad" || b.cell[i].title == "Short Line")
					{
						movePlayer(p,i);
					}
			}
    ...
    case 4: //Advance token to the nearest Railroad
	 		//and pay owner twice the rental to which he/she {he} 
	 		//is otherwise entitled. If Railroad is unowned, you may buy it from the Bank.
		bool out_of_bound =true;
		for(int i = p.position; i < b.sizeOfBoard; i++){
			if(b.cell[i].title == "Reading Railroads" || b.cell[i].title == "B. & O. Railroad" || b.cell[i].title == "Pennsylvania Railroad" || b.cell[i].title == "Short Line")
			{
				movePlayer(p,i);
				out_of_bound=false;
				if(b.cells[i].owner == null){
					...
					subtractMoney(p, 2*(b.cells[i].price));
					giveMoney(b.cells[i].owner, 2*(b.cells[i].price));
				}
		...
		if(out_of_bound==true){
			for(int i = 0; i < p.sizeOfBoard; i++){
				f(b.cell[i].title == "Reading Railroads" || b.cell[i].title == "B. & O. Railroad" || b.cell[i].title == "Pennsylvania Railroad" || b.cell[i].title == "Short Line")
					{
						movePlayer(p,i);
					}
			}
    ...
    case 5: //Bank pays you dividend of $50.
      	giveMoney(p,50);
     	break;
    case 6: //Get out of Jail Free. This card may be kept until needed.
			????
      	break;
    case 7: //Go Back Three {3} Spaces.
		movePlayer(p, -3);
		break;
    case 8: //Go to Jail. Go directly to Jail. Do not pass GO, do not collect $200.
		movePlayer(p, 11);
      	break;
    case 9: //"Make general repairs on all your property: For each house pay $25, For each hotel {pay} $100.
		int sum = 0;
		for(int i = 0; i < b.sizeOfBoard, i++){
			if(b.cells[i].owner == p){
				if(b.cells[i].impr.numOfHouses != 0) 
					sum += ((b.cells[i].impr.numOfHouses) * 25);
				else if(b.cells[i].impr.hotelExists == true) 
					sum += ((b.cells[i].impr.numOfHotels) * 100);
			}
		}
		subtractMoney(p, sum);
      	break;
    case 10: //Pay poor tax of $15
		subtractMoney(p, 15);
      	break;
    case 11: //Take a trip to Reading Railroad.If you pass Go, collect $200.
		bool out_of_bound =true;
		...
		for(int i = p.position; i < b.sizeOfBoard; i++){
			...
			if(b.cell[i].title == "Reading Railroads")
			{
				movePlayer(p,i);
				out_of_bound=false;
				if(pass_start == true)
		...
		if(out_of_bound==true){
	  	...
	  	break;
    case 12: //Take a walk on the Boardwalk. Advance token to Boardwalk. {Board Walk in both sentences}
		bool out_of_bound =true;
		for(int i = p.position; i < b.sizeOfBoard; i++){
			if(b.cell[i].title == "Boardwalk")
			{
				movePlayer(p,i);
				out_of_bound=false;
			}
    ...
    case 13: //You have been elected Chairman of the Board. Pay each player $50.
		...
		//pay???
      	break;
    case 14: //Your building {and} loan matures. Receive {Collect} $150.
		giveMoney(p, 150);
      	break;
